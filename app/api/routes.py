import json
import os
from datetime import datetime
from pathlib import Path

from flask import Blueprint, jsonify, session, request, current_app

from app import services, utils

api = Blueprint('api', __name__)
BYE_WEEKS_FILE = Path(__file__).resolve().parents[1] / 'data' / 'byeWeek2026.json'


def get_bye_weeks():
    try:
        with BYE_WEEKS_FILE.open('r', encoding='utf-8') as file:
            bye_weeks = json.load(file)
        return {
            team['id']: int(week)
            for week, teams in bye_weeks.items()
            for team in teams
        }
    except (OSError, TypeError, ValueError, KeyError) as error:
        current_app.logger.error('Unable to load bye week data: %s', error)
        return {}


@api.route('/player-status')
@utils.login_required
def player_status():
    user_id = session['user_id']
    show_best_ball = request.args.get('showBestBall', 'false').lower() == 'true'
    return jsonify(services.get_starters_with_status(user_id, show_best_ball=show_best_ball))


@api.route('/refresh-league-status')
@utils.login_required
def refresh_league_status():
    user_id = session['user_id']
    show_best_ball = request.args.get('showBestBall', 'false').lower() == 'true'
    return jsonify(services.get_starters_with_status(user_id, force_refresh=True, show_best_ball=show_best_ball))


@api.route('/cache-info')
@utils.login_required
def cache_info():
    cache_file = current_app.config['PLAYERS_CACHE_FILE']
    if not os.path.exists(cache_file):
        return jsonify(status='no_cache')
    mod_time = os.path.getmtime(cache_file)
    ttl = utils.get_cache_ttl()
    return jsonify(
        last_updated=datetime.fromtimestamp(mod_time).isoformat(),
        expires_at=datetime.fromtimestamp(mod_time + ttl).isoformat(),
        ttl_seconds=ttl,
        cache_size=os.path.getsize(cache_file),
        is_night=utils.is_night_time(),
        is_morning=utils.is_morning_time(),
    )


@api.route('/top-players')
@utils.login_required
def top_players():
    user_id = session['user_id']
    leagues = services.get_cached_leagues(user_id) or []
    all_players_data = services.get_all_players() or {}
    bye_weeks = get_bye_weeks()
    player_map = {}

    for league in leagues:
        if not league or 'league_id' not in league:
            continue
        league_id = league['league_id']
        rosters = services.get_cached_rosters(league_id) or []
        user_rosters = [roster for roster in rosters if roster and roster.get('owner_id') == user_id]

        for roster in user_rosters:
            for player_id in roster.get('players') or []:
                if not player_id:
                    continue
                player_data = all_players_data.get(player_id, {})
                team = player_data.get('team')
                bye_week = bye_weeks.get(team)
                player_name = player_data.get('full_name') or f'Player_{player_id[:6]}'

                if player_id not in player_map:
                    player_map[player_id] = {
                        'name': player_name,
                        'count': 0,
                        'leagues': [],
                        'position': player_data.get('position', '?'),
                        'team': team,
                        'bye_week': bye_week,
                        'injury_status': utils.format_status(player_data.get('injury_status') or 'Active'),
                    }

                player_map[player_id]['count'] += 1
                player_map[player_id]['leagues'].append({
                    'league_name': league.get('name', 'Unknown'),
                    'league_id': league_id,
                    'roster_id': roster.get('roster_id'),
                    'roster_position': services.get_roster_position(player_id, roster, league_id),
                })

    players_list = sorted(player_map.values(), key=lambda player: (-player['count'], player['name']))
    return jsonify(players_list[:current_app.config['TOPN']])


@api.route('/search-players')
@utils.login_required
def search_players():
    query = request.args.get('query', '').strip().lower()[:50]
    positions = request.args.getlist('positions')
    if not positions:
        return jsonify([])

    results = []
    for player_id, player in (services.get_all_players() or {}).items():
        full_name = player.get('full_name') or f"{player.get('first_name', '')} {player.get('last_name', '')}".strip()
        if not full_name or (query and query not in full_name.lower()):
            continue
        player_positions = player.get('fantasy_positions') or []
        if not any(position in positions for position in player_positions):
            continue
        results.append({
            'id': player_id,
            'name': full_name,
            'positions': player_positions,
            'status': player.get('status', 'Active'),
            'status_abbr': current_app.config['STATUS_CONFIG'].get(player.get('status'), {}).get('abbr', ''),
            'depth_chart_order': player.get('depth_chart_order'),
        })

    results.sort(key=lambda player: (player['depth_chart_order'] if player['depth_chart_order'] is not None else float('inf'), player['name']))
    return jsonify(results)


@api.route('/player-details')
@utils.login_required
def player_details():
    player_name = request.args.get('name', '').strip()
    if not player_name:
        return jsonify(error='Invalid player name'), 400

    user_id = session['user_id']
    leagues = services.get_cached_leagues(user_id) or []
    all_players = services.get_all_players() or {}
    player_id = next((pid for pid, data in all_players.items() if (data.get('full_name') or f"{data.get('first_name', '')} {data.get('last_name', '')}".strip()).lower() == player_name.lower()), None)
    if not player_id:
        return jsonify(error='Player not found'), 404

    player_data = all_players[player_id]
    leagues_with_player, leagues_without_players = [], []
    for league in leagues:
        league_id = league.get('league_id')
        if not league_id:
            continue
        found = False
        for roster in services.get_cached_rosters(league_id) or []:
            if player_id not in (roster.get('players') or []):
                continue
            found = True
            if roster.get('owner_id') == user_id:
                leagues_with_player.append({'league_id': league_id, 'league_name': league.get('name', 'Unknown'), 'roster_position': services.get_roster_position(player_id, roster, league_id), 'roster_id': roster.get('roster_id')})
            else:
                leagues_without_players.append({'league_id': league_id, 'league_name': league.get('name', 'Unknown'), 'status': 'TRADE'})
            break
        if not found:
            leagues_without_players.append({'league_id': league_id, 'league_name': league.get('name', 'Unknown'), 'status': 'F.A'})

    return jsonify({
        'player_name': player_name,
        'position': player_data.get('position', '?'),
        'injury_status': utils.format_status(player_data.get('injury_status') or 'Active'),
        'leagues': leagues_with_player,
        'other_leagues': leagues_without_players,
    })


@api.route('/refresh-players-cache')
@utils.login_required
def refresh_players_cache():
    try:
        cache_file = current_app.config['PLAYERS_CACHE_FILE']
        if os.path.exists(cache_file):
            os.remove(cache_file)
        services.get_all_players()
        return jsonify(success=True, message='Cache de jogadores atualizado com sucesso!')
    except Exception as error:
        current_app.logger.error('Erro ao forçar a atualização do cache de jogadores: %s', error)
        return jsonify(success=False, message='Ocorreu um erro ao atualizar o cache.'), 500


@api.route('/nfl-teams')
@utils.login_required
def nfl_teams():
    return jsonify(services.get_nfl_teams())


@api.route('/depth-chart/<team_abbr>')
@utils.login_required
def depth_chart(team_abbr):
    chart_data = services.get_nfl_depth_chart(team_abbr, request.args.get('league_id'))
    return jsonify({'chart': chart_data, 'current_user_id': session.get('user_id')})


@api.route('/all-leagues')
@utils.login_required
def get_all_leagues():
    return jsonify(services.get_cached_leagues(session['user_id']) or [])
